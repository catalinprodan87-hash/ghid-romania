...
// Standard icon: rounded square + location pin. `pad` keeps the glyph inside the
// maskable safe zone when rendered full-bleed.
function iconSvg({ rounded = true, pad = 0.5 } = {}) {
  const radius = rounded ? 96 : 0
  const cx = 256
  // Scale the pin down by `pad`-ish so maskable variants keep clear margins.
...
<svg xmlns="http://www.w3.org/2000/svg" width="512" height="512" viewBox="0 0 512 512">
  ...
  <g transform="translate(${t}, ${t}) scale(${scale})">
    <path d="M${cx} 96c-72 0-130 58-130 130 0 90 130 230 130 230s130-140 130-230c0-72-58-130-130-130z" fill="${ACCENT}" />
    <circle cx="${cx}" cy="226" r="55" fill="${PRIMARY_DARK}" />
  </g>
</svg>`
}
